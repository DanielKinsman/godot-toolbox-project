...
func get_state(uid:String):
    ...
    if nodes.size() == 0:
        Debug.error(Debug.LogCategory.PERSISTENCE, ["get_state", "Could not parse uid [", "UID:", uid, "]" ])
        return null

    # Cancel if uid_paths first node doesnt exists
    if !nodes[0] in _objs:
        Debug.error(Debug.LogCategory.PERSISTENCE, ["get_state", "Could not parse uid, invalid Node in Path [", "UID:", uid, ",", "Node:", nodes[0], "]" ])
        return null
...
func __try_descend(uid, tree, node):
    if tree is Array:
        if !node.is_valid_integer():
            Debug.error(Debug.LogCategory.PERSISTENCE, ["Could not parse uid, non-integer node on array [", "UID:", uid, ",", "Node:", node, "]" ])
            return null
        ...
        if tree.size() > inode:
            ...
            Debug.error(Debug.LogCategory.PERSISTENCE, ["Could not parse uid, out of bounds array access [", "UID:", uid, ",", "Node:", node, "]" ])
            return null
    elif tree is Dictionary:
        if tree.has(node):
            return tree[node]
        else:
            Debug.error(Debug.LogCategory.PERSISTENCE, ["Could not parse uid, invalid Node in Path [", "UID:", uid, ",", "Node:", node, "]" ])
            return null
...
func set_state(uid:String, val):
    var nodes = uid.split(".")
    if nodes.size() == 0:
        Debug.error(Debug.LogCategory.PERSISTENCE, ["set_state", "Could not parse uid [", "UID:", uid, "]" ])
    elif nodes.size() == 1:
        _objs[uid].set_state(val)
    else:
        # First node is entire json
        if !nodes[0] in _objs:
            Debug.error(Debug.LogCategory.PERSISTENCE, ["set_state", "Could not parse uid, invalid Node in Path [", "UID:", uid, ",", "Node:", nodes[0], "]" ])
            return false
        ...
        if tree is Dictionary and tree.has(last_node):
            ...
            Debug.error(Debug.LogCategory.PERSISTENCE, ["set_state", "Could not parse uid, invalid Node in Path [", "UID:", uid, ",", "Node:", last_node, "]" ])
            return false
...
class PersistentObj:
    ...
    func _save()->bool:
        ...
        if file.open(_get_save_path(), File.WRITE) == OK:
            ...
            Debug.log(Debug.LogCategory.PERSISTENCE, ["Wrote save [", "UID:", uid, ",", "Data:", string, "]" ])
            return true
        else:
            Debug.error(Debug.LogCategory.PERSISTENCE, ["Error opening PersistentObj File for writing [", "UID:", uid, "]" ])
        return false
    ...
    func _load_from_string(string:String)->bool:
        ...
        if json_result.error == OK:
            ...
            Debug.error(Debug.LogCategory.PERSISTENCE, ["Error parsing PersistentObj from json [", "UID:", uid, ",", "str:", string, "]" ])
            return false
        ...
        if _does_save_exist():
            ...
            if file.open(_get_save_path(), File.READ) == OK:
                ...
                if _load_from_string(string):
                    Debug.log(Debug.LogCategory.PERSISTENCE, ["Loaded save [", "UID:", uid, ",", "Data:", val, "]" ])
                    return true
            else:
                Debug.error(Debug.LogCategory.PERSISTENCE, ["Error opening PersistentObj File [", "UID:", uid, "]" ])
        return false
