...
func _ready():
	...
	if screen_history.size() != 1:
		Debug.error(Debug.LogCategory.SCREEN_MANAGER, ["Initial scene is not a is not inherited from Screen.tscn!"])
		return
...
func ___enter_screen(screen):
	...
	# Debug log & Signal
	Debug.log(Debug.LogCategory.SCREEN_MANAGER, ["Switched screen to", screen.name])
...
func ___exit_screen():
	...
	# Debug log & Signal
	Debug.log(Debug.LogCategory.SCREEN_MANAGER, ["Exited screen", screen_history[0].name])
...
func push_screen(screen_scene):
	...
	if !screen_inst is Screen:
		Debug.error(Debug.LogCategory.SCREEN_MANAGER, ["Tried pushing Screen, but it is not a PackedScene, inherited from Screen.tscn"])
		return
...
func pop_screen():
	if screen_history.size() < 2:
		Debug.error(Debug.LogCategory.SCREEN_MANAGER, ["Tried popping Screen, but the screen_history buffer would have been empty"])
		return
